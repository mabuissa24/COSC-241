...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # initialize booleans to false (has each corner been reached)
        # if start state is a corner, ADJUST booleans as necessary
    ...
    def booleanCheck(self, state):
        ...
        # loop through the corners, check if x, y = corner, make associated boolean true if so
...
def cornersHeuristic(state, problem):
    ...
    def distance(loc1, loc2):
        # Manhattan distance on coordinate plane (from current state to food)
        return abs(loc1[0] - loc2[0]) + abs(loc1[1] - loc2[1])

    loc = state[0]
    runningDistance = 0
    booleans = list(state[1])
    while True:
        corner = -1
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    width = foodGrid.packBits()[0]
    height = foodGrid.packBits()[1]
    foodmap = state[1]
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # Goal is when you find SOME piece of food (shallowest first in bfs)
